 ...
 *
 */
public class LoginPhoneAuth implements MuitiAuthenticatied {

	private static Logger log = LoggerFactory.getLogger(LoginPhoneAuth.class);
	...
	public AuthenticationInfo buildAuthenticationInfo(AuthenticationToken authenticationToken) {
		...
		String username = token.getUsername();
		User user = User.dao.getByName(username);
		SimpleAuthenticationInfo info = new SimpleAuthenticationInfo(user, "111111", null, "ShiroDbRealm");
		return info;
	}
