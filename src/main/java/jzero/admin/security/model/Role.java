...
//	}
	...
	public void insertCascade(Integer id, Integer authorityId) {
		Db.update("insert into sec_role_menu (role_id, menu_id) values (?,?)", id, authorityId);
	}
	
	public void deleteCascade(Integer id) {
		Db.update("delete from sec_role_menu where role_id = ?", id);
	}
	...
	public int findCountByRole(int role){
		String sql = "select * from sec_role_user where role_id=?";
		return Role.dao.find(sql,role).size();
	}

	//返回当前用户角色--》用户判断代理商充值设备还是普通用户充值：1：超级管理员；5：管理员；4：代理商；2：普通用户；
	public Integer getUserType(Integer userId) {
		if(findFirst("select * from sec_role_user where user_id=?",userId)!=null)
			return findFirst("select * from sec_role_user where user_id=?",userId).getInt("role_id");
		else
			return 2;
	}
